...
static viewvars_t p3view_old;
static viewvars_t p3view_new;
static viewvars_t p4view_old;
static viewvars_t p4view_new;
static viewvars_t sky1view_old;
static viewvars_t sky1view_new;
static viewvars_t sky2view_old;
static viewvars_t sky2view_new;
static viewvars_t sky3view_old;
static viewvars_t sky3view_new;
static viewvars_t sky4view_old;
static viewvars_t sky4view_new;
...
static void LerpPolyobjState(polyobj_t *pobj, fixed_t frac)
{
	for (unsigned int i = 0; i < pobj->numVertices; i++)
	{
		...
		pobj->vertices[i]->z = oldVert->z + R_LerpFixed(oldVert->z, newVert->z, frac);
	}
	...
	pobj->centerPt.z = pobj->oldCenterPt.z + R_LerpFixed(pobj->oldCenterPt.z, pobj->newCenterPt.z, frac);
}
#endif
...
void R_UpdateViewInterpolation(void)
{
	...
	p3view_old = p3view_new;
	p4view_old = p4view_new;
	sky1view_old = sky1view_new;
	sky2view_old = sky2view_new;
	sky3view_old = sky3view_new;
	sky4view_old = sky4view_new;
}
...
void R_SetViewContext(enum viewcontext_e _viewcontext)
{
	I_Assert(_viewcontext == VIEWCONTEXT_PLAYER1
			...
			|| _viewcontext == VIEWCONTEXT_PLAYER3
			|| _viewcontext == VIEWCONTEXT_PLAYER4
			|| _viewcontext == VIEWCONTEXT_SKY1
			|| _viewcontext == VIEWCONTEXT_SKY2
			|| _viewcontext == VIEWCONTEXT_SKY3
	...
	switch (viewcontext)
	{
		...
		case VIEWCONTEXT_SKY3:
			oldview = &sky3view_old;
			newview = &sky3view_new;
			break;
		case VIEWCONTEXT_SKY4:
			oldview = &sky4view_old;
			newview = &sky4view_new;
			break;
		default:
