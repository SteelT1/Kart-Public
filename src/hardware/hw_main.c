...
\
	...
	if (subsector)
	{
		...
		{
			if (line->linedef->special == HORIZONSPECIAL && R_PointOnSegSide(gr_viewx, gr_viewy, line) == 0)
			{
...
#undef SLOPEPARAMS
		...
		// Sky culling
		if (!gr_curline->polyseg || gr_curline->special != HORIZONSPECIAL) // Don't do it for polyobjects or horizon lines
		{
			// Sky Ceilings
